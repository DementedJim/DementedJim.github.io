...
CREATE TABLE IF NOT EXISTS `Work`.`Oblast` (
  `idOblast` INT UNSIGNED NOT NULL,
  PRIMARY KEY (`idOblast`),
  UNIQUE INDEX `idOblast_UNIQUE` (`idOblast` ASC) VISIBLE)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `Work`.`City` (
  ...
  `Oblast_name` INT UNSIGNED NULL,
  PRIMARY KEY (`idCity`),
  INDEX `Oblast_id_idx` (`Oblast_name` ASC) VISIBLE,
  UNIQUE INDEX `idCity_UNIQUE` (`idCity` ASC) VISIBLE,
  CONSTRAINT `fk_Oblast_id`
    FOREIGN KEY (`Oblast_name`)
...
CREATE TABLE IF NOT EXISTS `Work`.`Company` (
  ...
  INDEX `fk3_City_id_idx` (`City_id` ASC) VISIBLE,
  CONSTRAINT `fk_User_id`
    FOREIGN KEY (`User_id`)
    REFERENCES `Work`.`Users` (`idUsers`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk3_City_id`
    FOREIGN KEY (`City_id`)
...
CREATE TABLE IF NOT EXISTS `Work`.`Filial` (
  `idFilial` INT UNSIGNED NOT NULL,
  `City_id` INT UNSIGNED NULL,
...
CREATE TABLE IF NOT EXISTS `Work`.`Operators` (
  `idOperators` INT UNSIGNED NOT NULL,
...
CREATE TABLE IF NOT EXISTS `Work`.`Vacancy` (
  ...
  INDEX `fk_Filial_id_idx` (`Filial_id` ASC) VISIBLE,
  CONSTRAINT `fk2_Company_id`
    FOREIGN KEY (`Company_id`)
    REFERENCES `Work`.`Company` (`idCompany`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk2_Operator_id`
    FOREIGN KEY (`Operator_id`)
    REFERENCES `Work`.`Operators` (`idOperators`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk2_Filial_id`
    FOREIGN KEY (`Filial_id`)
    REFERENCES `Work`.`Filial` (`idFilial`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk2_City_id`
    FOREIGN KEY ()
    REFERENCES `Work`.`City` ()
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `Work`.`Resume` (
  ...
  CONSTRAINT `fk1_City_id`
    FOREIGN KEY (`City_id`)
    REFERENCES `Work`.`City` (`idCity`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk1_Filial_id`
    FOREIGN KEY (`Filial_id`)
    REFERENCES `Work`.`Filial` (`idFilial`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk1_Operator_id`
    FOREIGN KEY (`Operator_id`)
    REFERENCES `Work`.`Operators` (`idOperators`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
